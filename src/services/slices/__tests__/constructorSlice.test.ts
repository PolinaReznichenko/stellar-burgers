...
describe('Тест редьюсера слайса [constructorSlice]', () => {
    ...
    test('Добавляется новый ингредиент', () => {
        ...
        const addNewIngredient = constructorReducer({...initialState}, addIngredient({
            _id: '643d69a5c3f7b9001cfa0947',
            name: 'Плоды Фалленианского дерева',
            type: 'main',
            proteins: 20,
        ...
        expect(addNewIngredient.ingredients[0]).toEqual(expect.objectContaining(ingredientInConstructor));
    });

    test('Добавляется булка', () => {
        //Добавляем булку в конструктор с помощью экшена setBun
        const addBun = constructorReducer({...initialState}, setBun({
            _id: '643d69a5c3f7b9001cfa093c',
            name: 'Краторная булка N-200i',
            type: 'bun',
            proteins: 80,
    ...
    test('Удаляется ингредиент', () => {
        ...
        const deleteIngredient = constructorReducer(initialConstructorState, removeIngredient('111'));
    ...
    test('Сортируются ингредиенты (кроме булок)', () => {
        ...
        const moveIngredients = constructorReducer(initialConstructorState, moveIngredient({from: 1, to: 0}));
    ...
    test('Очищается конструктор бургера', () => {
        ...
        const clearBurgerConstructor = constructorReducer(initialConstructorState, clearConstructor());
